// import React, { useState } from 'react';
// import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
// import Button from "@/components/Button";
// import {
//   faPlus, faTrash, faEdit, faMagnifyingGlass,
//   faArrowDownAZ,
//   faDownload,
//   faFileImport, faFilter, faTrashAlt,
//   faRotate
// } from "@fortawesome/free-solid-svg-icons"; import '@/styles/buttonFrame.css';
// import Popup from '@/components/Popup'; // Import the Popup component
// import axios from "axios";
// import { useEffect } from "react";

// const ButtonFrame2 = () => {
//   const [vendorData, setVendorData] = useState({
//     vendor_name: "",
//     reliability_score: "",
//     delivery_time: "",
//     vendor_phone: "",
//     category: "",
//     product_barcode: "",
//   });
//   // Debugging: See updates in real-time
//   useEffect(() => {
//     console.log("📊 Vendor Data Updated:", vendorData);
//   }, [vendorData]);


//   const [popupConfig, setPopupConfig] = useState({
//     isOpen: false,
//     head: '', // Main heading
//     title: '', // Subheading
//     content: null, // Dynamic content
//   });
//   // Function to open the popup
//   const openPopup = (head, title, content) => {
//     setPopupConfig({
//       isOpen: true,
//       head,
//       title,
//       content,
//     });
//   };
//   // Function to close the popup
//   const closePopup = () => {
//     setPopupConfig({
//       isOpen: false,
//       head: '',
//       title: '',
//       content: null,
//     });
//   };
//   // Handle Input Change
//   // Handle Input Change

//   const handleInputChange = (e) => {
//     const { name, value } = e.target;

//     setVendorData((prevData) => ({
//       ...prevData,
//       [name]: value,
//     }));

//     console.log(`📝 ${name}: ${value}`); // Logs each input update
//   };

//   const handleSubmit = async (e) => {
//     e.preventDefault(); // Prevent default form submission

//     console.log("📤 Submitting Form Data:", vendorData); // Now logging actual formData
//     if (!vendorData.vendor_name || !vendorData.vendor_phone) {
//       alert("Please fill out all required fields.");
//       return;
//     }

//     alert(`Vendor Added: ${vendorData.vendor_name}`);
//     setVendorData({ vendor_name: "", reliability_score: "", delivery_time: "", vendor_phone: "", category: "" });
//     // Validation
//     // if (!formData.vendor_name || !formData.reliability_score || !formData.delivery_time ||
//     //   !formData.vendor_phone || !formData.category || !formData.product_barcode) {
//     //   alert("Please fill out all fields before submitting.");
//     //   return;
//     // }

//     // try {
//     //   const response = await axios.post("http://127.0.0.1:8000/aiventory/add-vendor/", formData);
//     //   console.log("✅ Response from Backend:", response.data); // Log backend response
//     //   console.log("✅ Response from Backend:", response.data);

//     //   // Reset form and close popup
//     //   setFormData({
//     //     vendor_name: "",
//     //     reliability_score: "",
//     //     delivery_time: "",
//     //     vendor_phone: "",
//     //     category: "",
//     //     product_barcode: "",
//     //   });
//     //   closePopup();
//     // } catch (error) {
//     //   console.error("❌ Error adding vendor:", error.response?.data || error.message);
//     //   alert("Failed to add vendor. Please try again.");
//     // }
//   };

//   // Static Form Fields
//   const addVendorContent = (
//     <div>
//       <h3>Add a Vendor</h3>
//       <form onSubmit={handleSubmit}> {/* Moved onSubmit here */}

//         <div>
//           <label>Vendor Name:</label>
//           <input type="text" name="vendor_name" value={vendorData.vendor_name} onChange={handleInputChange} />

//         </div>
//         <div>
//           <label>Reliability Score:</label>
//           <input type="number"
//             name="reliability_score"
//             value={vendorData.reliability_score} // Bind value
//             onChange={handleInputChange} />
//         </div>
//         <div>
//           <label>Delivery Time:</label>
//           <input type="text"
//             name="delivery_time"
//             value={vendorData.delivery_time} // Bind value
//             onChange={handleInputChange} />
//         </div>
//         <div>
//           <label>Vendor Phone:</label>
//           <input type="text"
//             name="vendor_phone"
//             value={vendorData.vendor_phone} // Bind value
//             onChange={handleInputChange} />
//         </div>
//         <div>
//           <label>Category:</label>
//           <input type="text"
//             name="category"
//             value={vendorData.category} // Bind value
//             onChange={handleInputChange} />
//         </div>
//         <div>
//           <label>Product Barcode:</label>
//           <input type="text"
//             name="product_barcode"
//             value={vendorData.product_barcode} // Bind value
//             onChange={handleInputChange} />
//         </div>
//         <button type="submit">Confirm</button> {/* Changed to type="submit" */}
//       </form>

//     </div>
//   );
//   const deleteVendorContent = (
//     <div>
//       <h3>Delete a Vendor</h3>
//       {/* Add your delete confirmation or input fields here */}
//     </div>
//   );

//   const updateVendorContent = (
//     <div>
//       <h3>Update a Vendor</h3>
//       {/* Add your update form or input fields here */}
//     </div>
//   );
//   const searchVendorContent = (
//     <div>
//       <h3>Search a Vendor</h3>
//       {/* Add your update form or input fields here */}
//     </div>
//   );
//   const buttonData = [
//     {
//       id: 1,
//       text: "Add a Vendor",
//       icon: faPlus,
//       onClick: () => openPopup("Vendor Management", "Add a Vendor", addVendorContent),
//     },
//     {
//       id: 2,
//       text: "Delete a Vendor",
//       icon: faTrashAlt,
//       onClick: () => openPopup("Vendor Management", "Delete a Vendor", deleteVendorContent),
//     },
//     {
//       id: 3,
//       text: "Update a Vendor",
//       icon: faRotate,
//       onClick: () => openPopup("Vendor Management", "Update a Vendor", updateVendorContent),
//     },
//     {
//       id: 4,
//       text: "Search a Vendor",
//       icon: faMagnifyingGlass,
//       onClick: () => openPopup("Vendor Management", "Search a Vendor", searchVendorContent),
//     },
//   ];
//   return (
//     <>

//       <div className="buttonContainer">
//         {buttonData.map((button) => (
//           <Button
//             key={button.id}
//             text={button.text}
//             icon={button.icon}
//             onClick={button.onClick} // Pass the onClick handler

//           />
//         ))}
//       </div>

//       {/* Render the Popup if isPopupOpen is true */}
//       {popupConfig.isOpen && (
//         <Popup
//           head={popupConfig.head}

//           title={popupConfig.title}
//           content={popupConfig.content}
//           onClose={closePopup}
//         />
//       )}
//     </>

//   );
// };

// export default ButtonFrame2;


import React, { useState,useRef } from 'react';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import Button from "@/components/Button";
import {
  faPlus, faTrash, faEdit, faMagnifyingGlass,
  faArrowDownAZ,
  faDownload,
  faFileImport, faFilter, faTrashAlt,
  faRotate
} from "@fortawesome/free-solid-svg-icons"; import '@/styles/buttonFrame.css';
import Popup from '@/components/Popup'; // Import the Popup component
import axios from "axios";
import { useEffect } from "react";

const ButtonFrame2 = () => {
   // Create refs for each input field
   const formRef = useRef({
    vendor_name: React.createRef(),
    reliability_score: React.createRef(),
    delivery_time: React.createRef(),
    vendor_phone: React.createRef(),
    category: React.createRef(),
    product_barcode: React.createRef()
  });
  // Debugging: See updates in real-time

  const [popupConfig, setPopupConfig] = useState({
    isOpen: false,
    head: '', // Main heading
    title: '', // Subheading
    content: null, // Dynamic content
  });
  // Function to open the popup
  const openPopup = (head, title, content) => {
    setPopupConfig({
      isOpen: true,
      head,
      title,
      content,
    });
  };
  // Function to close the popup
  const closePopup = () => {
    setPopupConfig({
      isOpen: false,
      head: '',
      title: '',
      content: null,
    });
  };
  // Handle Input Change
  // Handle Input Change


  const handleSubmit = async (e) => {
    e.preventDefault(); // Prevent default form submission

   // Get all form values from refs
   const formData = {
    vendor_name: formRef.current.vendor_name.current.value,
    reliability_score: formRef.current.reliability_score.current.value,
    delivery_time: formRef.current.delivery_time.current.value,
    vendor_phone: formRef.current.vendor_phone.current.value,
    category: formRef.current.category.current.value,
    product_barcode: formRef.current.product_barcode.current.value
  };

  console.log("Submitting:", formData);

   
  if (!formData.vendor_name || !formData.vendor_phone) {
    alert("Please fill out all required fields.");
    return;
  }

  try {
    const response = await axios.post("http://127.0.0.1:8000/aiventory/add-vendor/", formData);
    console.log("Response:", response.data);
    alert(`Vendor ${formData.vendor_name} added successfully!`);
    
    // Clear all form fields
    Object.values(formRef.current).forEach(ref => {
      ref.current.value = '';
    });
    
    closePopup();
  } catch (error) {
    console.error("Error:", error);
    alert("Failed to add vendor. Please try again.");
  }
};
  // Static Form Fields
  const addVendorContent = (
    <div>
      <h3>Add a Vendor</h3>
      <form onSubmit={handleSubmit}> {/* Moved onSubmit here */}

        <div>
          <label>Vendor Name:</label>
          <input 
            type="text" 
            name="vendor_name" 
            ref={formRef.current.vendor_name} 
            defaultValue="" 
          />
        </div>
        <div>
          <label>Reliability Score:</label>
          <input 
            type="number"
            name="reliability_score"
            ref={formRef.current.reliability_score}
            defaultValue=""
          />
        </div>
        <div>
          <label>Delivery Time:</label>
          <input 
            type="text"
            name="delivery_time"
            ref={formRef.current.delivery_time}
            defaultValue=""
          />
        </div>
        <div>
          <label>Vendor Phone:</label>
          <input 
            type="text"
            name="vendor_phone"
            ref={formRef.current.vendor_phone}
            defaultValue=""
          />
        </div>
        <div>
          <label>Category:</label>
          <input 
            type="text"
            name="category"
            ref={formRef.current.category}
            defaultValue=""
          />
        </div>
        <div>
          <label>Product Barcode:</label>
          <input 
            type="text"
            name="product_barcode"
            ref={formRef.current.product_barcode}
            defaultValue=""
          />
        </div>
        <button type="submit">Confirm</button> {/* Changed to type="submit" */}
      </form>

    </div>
  );
  const deleteVendorContent = (
    <div>
      <h3>Delete a Vendor</h3>
      {/* Add your delete confirmation or input fields here */}
    </div>
  );

  const updateVendorContent = (
    <div>
      <h3>Update a Vendor</h3>
      {/* Add your update form or input fields here */}
    </div>
  );
  const searchVendorContent = (
    <div>
      <h3>Search a Vendor</h3>
      {/* Add your update form or input fields here */}
    </div>
  );
  const buttonData = [
    {
      id: 1,
      text: "Add a Vendor",
      icon: faPlus,
      onClick: () => openPopup("Vendor Management", "Add a Vendor", addVendorContent),
    },
    {
      id: 2,
      text: "Delete a Vendor",
      icon: faTrashAlt,
      onClick: () => openPopup("Vendor Management", "Delete a Vendor", deleteVendorContent),
    },
    {
      id: 3,
      text: "Update a Vendor",
      icon: faRotate,
      onClick: () => openPopup("Vendor Management", "Update a Vendor", updateVendorContent),
    },
    {
      id: 4,
      text: "Search a Vendor",
      icon: faMagnifyingGlass,
      onClick: () => openPopup("Vendor Management", "Search a Vendor", searchVendorContent),
    },
  ];
  return (
    <>

      <div className="buttonContainer">
        {buttonData.map((button) => (
          <Button
            key={button.id}
            text={button.text}
            icon={button.icon}
            onClick={button.onClick} // Pass the onClick handler

          />
        ))}
      </div>

      {/* Render the Popup if isPopupOpen is true */}
      {popupConfig.isOpen && (
        <Popup
          head={popupConfig.head}

          title={popupConfig.title}
          content={popupConfig.content}
          onClose={closePopup}
        />
      )}
    </>

  );
};

export default ButtonFrame2;